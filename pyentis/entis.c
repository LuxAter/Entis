...
static PyObject* entis_connection_valid(){
  if(connection_ == NULL){
    Py_RETURN_FALSE;
  }
  Py_RETURN_TRUE;
}
static PyObject* entis_flush() {xcb_flush(connection_); Py_RETURN_NONE;}

static PyObject* entis_update() {entis_copy_pixmap(); entis_flush(); Py_RETURN_NONE;}
...
PyMODINIT_FUNC PyInit_entis(void) {
  /* return PyModule_Create(&entismodule); */
